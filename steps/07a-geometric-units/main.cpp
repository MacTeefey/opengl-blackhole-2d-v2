...
struct Ray {
    ...
    Ray(float x, float y, float vx, float vy) {
        ...
        // Calculate conserved quantities (RS = 1 in geometric units)
        // f = 1 - 1/r (simplified from f = 1 - rs/r)
        L = r * r * v_phi;
        float f{1.0f - 1.0f / r};
    ...
    void updateDeflection() {
        ...
        if (deflection > M_PI) {
            deflection = 2.0f * M_PI - deflection;
        }
    }
};
...
void geodesicRHS(const Ray& ray, float rhs[4]) {
    ...
    // Simplified equations with RS = 1
    rhs[2] = -(1.0f / (2.0f * r * r)) * f * (dt_dlambda * dt_dlambda)
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glLoadIdentity();
        glOrtho(-viewWidth, viewWidth, -viewHeight, viewHeight, -1.0f, 1.0f);
